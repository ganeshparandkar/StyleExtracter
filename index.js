...
let a = cssbeautify(not_formatted, options);
document.getElementById('formatted_css').innerHTML = a;
    ...
    if (line.includes('class')) {
      ...
      for (let j = 0; j < b.length; j++) {
        ...
        if (element.includes('class')) {
          classNameList.push(b[j + 1]);
        }
      }
    }
  }
  let allClasses = classNameList.join(' ').split(' ');
  if (allClasses) {
    return allClasses
  }else{
    return null;
...
function Copythis() {
  ...
  elem.value = document.getElementById('formatted_css').innerHTML;
